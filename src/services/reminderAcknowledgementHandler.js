...
export async function handleReminderAcknowledgement(reminderId, sessionId) {
  try {
    ...
    const updateLogs = await prisma.reminderDeliveryLog.updateMany({
      where: {
        reminderId,
        sessionId,
    ...
    if (next) {
      ...
      logger.info(
        `✅ One-time reminder "${reminder.title}" marked as completed`,
      );
    }
  } catch (err) {
    console.log("ReminderAcknowledgementHandler Error: ", err);
  }
}
